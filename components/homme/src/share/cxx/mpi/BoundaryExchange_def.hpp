...
#ifndef HOMMEXX_BOUNDARY_EXCHANGE_DEF_HPP
#define HOMMEXX_BOUNDARY_EXCHANGE_DEF_HPP
...
{
  using PT = std::remove_reference_t<decltype(*a.data()->data())>;
  a = ExecViewManaged<ExecViewManaged<PT[NP][NP][NUM_LEV]>**>("3d fields", ne, a2);
  b = ExecViewManaged<ExecViewManaged<PT[NP][NP][NUM_LEV_P]>**>("3d interface fields", ne, b2);
}

template<typename ST>
BoundaryExchangeST<ST>::BoundaryExchangeST()
{
  ...
  if constexpr (Sacado::IsFad<ST>::value) {
    ...
    constexpr int fad_sz = sizeof(ST) / sizeof(Real);
  ...
  m_scalar_size = sizeof(ST);
}

template<typename ST>
BoundaryExchangeST<ST>::
...
template<typename ST>
void BoundaryExchangeST<ST>::set_num_fields (const int num_1d_fields, const int num_2d_fields, const int num_3d_fields, const int num_3d_int_fields)
{
  // We don't allow to call this method twice in a row. If you want to change the number of fields,
  // you need to call clean_up first, to get a fresh new BoundaryExchangeST.
...
template<typename ST>
void BoundaryExchangeST<ST>::clean_up()
{
  if (m_cleaned_up) {
...
template<typename ST>
void BoundaryExchangeST<ST>::exchange () {
  exchange(nullptr);
}

template<typename ST>
void BoundaryExchangeST<ST>::exchange (ExecViewUnmanaged<const Real * [NP][NP]> rspheremp) {
  exchange(&rspheremp);
}

template<typename ST>
void BoundaryExchangeST<ST>::exchange (const ExecViewUnmanaged<const Real * [NP][NP]>* rspheremp)
{
...
template<typename ST>
void BoundaryExchangeST<ST>::exchange_min_max ()
{
  // Check that the registration has completed first
  assert (m_registration_completed);
...
pack (const ExecViewUnmanaged<const HaloExchangeUnstructuredConnectionInfo*> ucon,
      ...
      const ExecViewUnmanaged<ExecViewManaged<ST[NP][NP]>**> fields_2d,
      const ExecViewUnmanaged<ExecViewUnmanaged<ST*>**> send_2d_buffers,
...
template <typename ST, int NUM_LEV_PACKS, bool partial_column=false>
static void
pack (const ExecViewUnmanaged<const HaloExchangeUnstructuredConnectionInfo*> ucon,
      const ExecViewUnmanaged<const int*> ucon_ptr,
      const ExecViewUnmanaged<ExecViewManaged<PackType<ST>[NP][NP][NUM_LEV_PACKS]>**> fields_3d,
      const ExecViewUnmanaged<ExecViewUnmanaged<PackType<ST>**>**> send_3d_buffers,
...
template<typename ST>
void BoundaryExchangeST<ST>::pack_and_send ()
{
  ...
  if (m_num_2d_fields > 0)
    pack<ST>(ucon, ucon_ptr, m_2d_fields, m_send_2d_buffers, m_num_elems,
         m_num_2d_fields);
  // ...then pack 3d fields (if any)...
  if (m_num_3d_fields > 0) {
    if (m_3d_nlev_pack_d.size() > 0)
      pack<ST,NUM_LEV, true>(ucon, ucon_ptr, m_3d_fields, m_send_3d_buffers,
                          m_num_elems, m_num_3d_fields, &m_3d_nlev_pack_d);
    else
      pack<ST,NUM_LEV>(ucon, ucon_ptr, m_3d_fields, m_send_3d_buffers,
                    m_num_elems, m_num_3d_fields);
  }
  // ...then pack 3d interface fields (if any)
  if (m_num_3d_int_fields > 0)
    pack<ST,NUM_LEV_P>(ucon, ucon_ptr, m_3d_int_fields, m_send_3d_int_buffers,
...
template<typename ST>
void BoundaryExchangeST<ST>::recv_and_unpack () {
...
unpack (const ExecViewUnmanaged<const HaloExchangeUnstructuredConnectionInfo*> ucon,
        const ExecViewUnmanaged<const int*> ucon_ptr,
        const ExecViewUnmanaged<ExecViewManaged<ST[NP][NP]>**> fields_2d,
        const ExecViewUnmanaged<ExecViewUnmanaged<ST*>**> recv_2d_buffers,
...
unpack (const ExecViewUnmanaged<const HaloExchangeUnstructuredConnectionInfo*> ucon,
        const ExecViewUnmanaged<const int*> ucon_ptr,
        const ExecViewUnmanaged<ExecViewManaged<PackType<ST>[NP][NP][NUM_LEV_PACKS]>**> fields_3d,
        const ExecViewUnmanaged<ExecViewUnmanaged<PackType<ST>**>**> recv_3d_buffers,
        const ExecViewUnmanaged<const Real * [NP][NP]>* rspheremp,
        const int num_elems, const int num_3d_fields,
        ExecViewManaged<int*>* nlev_packs_ = nullptr) {
  assert(partial_column == (nlev_packs_ != nullptr));
...
template<typename ST>
void BoundaryExchangeST<ST>::recv_and_unpack (const ExecViewUnmanaged<const Real * [NP][NP]>* rspheremp)
{
  ...
  if (m_num_2d_fields>0)
    unpack<ST>(ucon, ucon_ptr, m_2d_fields, m_recv_2d_buffers, rspheremp, m_num_elems,
           m_num_2d_fields);
  // ...then unpack 3d fields (if any)...
  if (m_num_3d_fields>0) {
    if (m_3d_nlev_pack_d.size() > 0)
      unpack<ST,NUM_LEV, true>(ucon, ucon_ptr, m_3d_fields, m_recv_3d_buffers, rspheremp,
                            m_num_elems, m_num_3d_fields, &m_3d_nlev_pack_d);
    else
      unpack<ST,NUM_LEV>(ucon, ucon_ptr, m_3d_fields, m_recv_3d_buffers, rspheremp,
                      m_num_elems, m_num_3d_fields);
  }
  // ...then unpack 3d interface fields (if any).
  if (m_num_3d_int_fields > 0)
    unpack<ST,NUM_LEV_P>(ucon, ucon_ptr, m_3d_int_fields, m_recv_3d_int_buffers, rspheremp,
                      m_num_elems, m_num_3d_int_fields);
  Kokkos::fence();
...
static void pack_min_max (
  ...
  const ExecViewUnmanaged<const int*> ucon_ptr,
  const ExecViewUnmanaged<ExecViewManaged<PackType<ST>[2][NUM_LEV]>**> fields_1d,
  const ExecViewUnmanaged<ExecViewUnmanaged<PackType<ST>[2][NUM_LEV]>**> send_1d_buffers,
...
template<typename ST>
void BoundaryExchangeST<ST>::pack_and_send_min_max ()
{
  // The registration MUST be completed by now
  // Note: this also implies connectivity and buffers manager are valid
  assert (m_registration_completed);
  ...
  pack_min_max<ST>(m_connectivity->get_d_ucon(), m_connectivity->get_d_ucon_ptr(),
...
static void unpack_min_max (
  ...
  const ExecViewUnmanaged<const int*> ucon_ptr,
  const ExecViewUnmanaged<ExecViewManaged<PackType<ST>[2][NUM_LEV]>**> fields_1d,
  const ExecViewUnmanaged<ExecViewUnmanaged<PackType<ST>[2][NUM_LEV]>**> recv_1d_buffers,
  const int num_elems, const int num_1d_fields)
{
  if (OnGpu<ExecSpace>::value) {
    const ConnectionHelpers helpers;
...
template<typename ST>
void BoundaryExchangeST<ST>::recv_and_unpack_min_max ()
{
  // The registration MUST be completed by now
  // Note: this also implies connectivity and buffers manager are valid
  assert (m_registration_completed);
  ...
  unpack_min_max<ST>(m_connectivity->get_d_ucon(), m_connectivity->get_d_ucon_ptr(),
...
template<typename ST>
void BoundaryExchangeST<ST>::build_buffer_views_and_requests()
{
  ...
  using local_buf_ptr_type = ST*;
  using local_buf_val_type = ST;
  ...
  h_all_send_buffers[etoi(ConnectionSharing::LOCAL)]   = reinterpret_cast<ST*>(bm->get_local_buffer());
  h_all_send_buffers[etoi(ConnectionSharing::SHARED)]  = reinterpret_cast<ST*>(bm->get_send_buffer());
  h_all_send_buffers[etoi(ConnectionSharing::MISSING)] = reinterpret_cast<ST*>(bm->get_blackhole_send_buffer());
  h_all_recv_buffers[etoi(ConnectionSharing::LOCAL)]   = reinterpret_cast<ST*>(bm->get_local_buffer());
  h_all_recv_buffers[etoi(ConnectionSharing::SHARED)]  = reinterpret_cast<ST*>(bm->get_recv_buffer());
  h_all_recv_buffers[etoi(ConnectionSharing::MISSING)] = reinterpret_cast<ST*>(bm->get_blackhole_recv_buffer());
  ...
  for (size_t k = 0; k < nconn; ++k) {
    ...
    for (int f = 0; f < m_num_1d_fields; ++f) {
      h_send_1d_buffers(f, i) = ExecViewUnmanaged<PT[2][NUM_LEV]>(
        reinterpret_cast<PT*>(send_buffer.get() + h_buf_offset[info.sharing]));
      h_recv_1d_buffers(f, i) = ExecViewUnmanaged<PT[2][NUM_LEV]>(
        reinterpret_cast<PT*>(recv_buffer.get() + h_buf_offset[info.sharing]));
      h_buf_offset[info.sharing] += h_increment_1d[info.kind]*NUM_LEV*VECTOR_SIZE;
    }
    for (int f = 0; f < m_num_2d_fields; ++f) {
      h_send_2d_buffers(f, i) = ExecViewUnmanaged<ST*>(
        send_buffer.get() + h_buf_offset[info.sharing], helpers.CONNECTION_SIZE[info.kind]);
      h_recv_2d_buffers(f, i) = ExecViewUnmanaged<ST*>(
  ...
  auto send_ptr = reinterpret_cast<ST*>(bm->get_mpi_send_buffer());
  auto recv_ptr = reinterpret_cast<ST*>(bm->get_mpi_recv_buffer());
  int offset = 0;
...
template<typename ST>
void BoundaryExchangeST<ST>::clear_buffer_views_and_requests ()
{
...
#endif // HOMMEXX_BOUNDARY_EXCHANGE_DEF_HPP
