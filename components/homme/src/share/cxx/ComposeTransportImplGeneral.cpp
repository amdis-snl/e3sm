...
void ComposeTransportImpl::init_boundary_exchanges () {
  ...
  for (int i = 0; i < Q_NUM_TIME_LEVELS; ++i) {
    ...
    be->m_label = std::string("ComposeTransport-qdp-DSS-" + std::to_string(i));
  ...
  if (m_data.trajectory_nsubstep == 0) {
    for (int i = 0; i < 2; ++i) {
      ...
      be->m_label = std::string("ComposeTransport-v-DSS-" + std::to_string(i));
      be->m_diagnostics_level = sp.internal_diagnostics_level;
      be->set_buffers_manager(bm_exchange);
  ...
  if (m_data.hv_q > 0 && m_data.nu_q > 0) {
    for (int i = 0; i < 2; ++i) {
      ...
      be->m_label = std::string("ComposeTransport-q-HV-" + std::to_string(i));
      be->m_diagnostics_level = sp.internal_diagnostics_level;
      be->set_buffers_manager(bm_exchange);
