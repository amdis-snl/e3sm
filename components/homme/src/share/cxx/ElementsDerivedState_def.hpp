...
#ifndef HOMME_ELEMENTS_DERIVED_STATE_DEF_HPP
#define HOMME_ELEMENTS_DERIVED_STATE_DEF_HPP
...
void ElementsDerivedStateST<ST>::init(const int num_elems) {
  // Sanity check
  assert (num_elems>0);

  m_num_elems = num_elems;

  m_omega_p = ExecViewManaged<PT * [NP][NP][NUM_LEV]>("Omega P", m_num_elems);

  m_vn0 = ExecViewManaged<PT * [2][NP][NP][NUM_LEV]>("Derived Lateral Velocities", m_num_elems);
  m_vstar = ExecViewManaged<PT * [2][NP][NP][NUM_LEV]>("v at tracer time step start", m_num_elems);

  m_eta_dot_dpdn = ExecViewManaged<PT * [NP][NP][NUM_LEV_P]>("eta_dot_dpdn", m_num_elems);

  m_dp                = ExecViewManaged<PT * [NP][NP][NUM_LEV]>("derived_dp", m_num_elems);
  m_divdp             = ExecViewManaged<PT * [NP][NP][NUM_LEV]>("derived_divdp", m_num_elems);
  m_divdp_proj        = ExecViewManaged<PT * [NP][NP][NUM_LEV]>("derived_divdp_proj", m_num_elems);
  m_dpdiss_biharmonic = ExecViewManaged<PT * [NP][NP][NUM_LEV]>("derived_dpdiss_biharmonic", m_num_elems);
  m_dpdiss_ave        = ExecViewManaged<PT * [NP][NP][NUM_LEV]>("derived_dpdiss_ave", m_num_elems);
}

template<typename ST>
void ElementsDerivedStateST<ST>::randomize(const int seed, const Real dp3d_min) {
  ...
  ExecViewManaged<PT *[NP][NP][NUM_LEV]> delta_eta("",nslots);
...
#endif // HOMME_ELEMENTS_DERIVED_STATE_DEF_HPP
