...
// Verify run_JV by comparing against a finite-difference Jacobian-vector product.
// J*V ≈ (DIRK(x + h*V) - DIRK(x)) / h for small h.
TEST_CASE ("dirk_jv_testing") {
  using DxFadType = DxFadTypeDirk;
  using dfiFAD = DirkFunctorImplST<DxFadType>;

  const int n0 = 1, np1 = 2, nm1 = 0;
  const int np = NP, nlev = dfi::num_phys_lev;
  Real dt2 = 0.1;
  Real alphadt_nm1 = 0.0, alphadt_n0 = 0.3 * dt2;

  auto& s = Session::singleton();
  const auto& hvcoord = s.h();
  auto& r = s.r;
  auto& e_real = s.e();
  const int nelemd = s.nelemd;

  // Create FAD-typed elements
  auto& c = Context::singleton();
  auto& elems_dx = c.create<ElementsST<DxFadType>>();
  elems_dx.init(nelemd, false, true, PhysicalConstants::rearth0);
  elems_dx.m_geometry = e_real.m_geometry;

  // Create FAD DIRK functor
  dfiFAD dirk_dx(nelemd);
  FunctorsBuffersManager fbm_dx;
  fbm_dx.request_size(dirk_dx.requested_buffer_size());
  fbm_dx.allocate();
  dirk_dx.init_buffers(fbm_dx);

  // Create Real DIRK functor
  dfi dirk_real(nelemd);
  FunctorsBuffersManager fbm_real;
  init(dirk_real, fbm_real);

  // Find a valid problem using the same trial-loop approach as dirk_toplevel_testing
  bool good = false;
  for (int trial = 0; trial < 100; ++trial) {
    init_elems(2, nelemd, r, hvcoord, e_real);

    // Save state before trial DIRK run
    decltype(e_real.m_state.m_w_i) w_i_save("w_i_save", nelemd);
    decltype(e_real.m_state.m_phinh_i) phi_save("phi_save", nelemd);
    Kokkos::deep_copy(w_i_save, e_real.m_state.m_w_i);
    Kokkos::deep_copy(phi_save, e_real.m_state.m_phinh_i);

    dirk_real.run(nm1, alphadt_nm1, n0, alphadt_n0, np1, dt2,
                  e_real, hvcoord, true /* BFB */);
    Kokkos::fence();

    const auto w2m   = cmvdc(e_real.m_state.m_w_i);
    const auto phi2m = cmvdc(e_real.m_state.m_phinh_i);

    bool ok = true;
    for (int ie = 0; ie < nelemd && ok; ++ie)
      for (int i = 0; i < np && ok; ++i)
        for (int j = 0; j < np && ok; ++j) {
          auto* pw = &w2m(ie,np1,i,j,0)[0];
          auto* pp = &phi2m(ie,np1,i,j,0)[0];
          for (int k = 0; k <= nlev; ++k)
            if (std::isnan(ADValue(pw[k])) || std::isinf(ADValue(pw[k])) ||
                std::isnan(ADValue(pp[k])) || std::isinf(ADValue(pp[k])))
              ok = false;
          for (int k = 0; k < nlev; ++k)
            if (pp[k] <= pp[k+1]) ok = false;
        }

    // Restore state
    Kokkos::deep_copy(e_real.m_state.m_w_i, w_i_save);
    Kokkos::deep_copy(e_real.m_state.m_phinh_i, phi_save);

    if (!ok) { dt2 *= 0.99; alphadt_n0 *= 0.99; continue; }
    good = true;
    break;
  }

  if (!good) {
    std::cout << "dirk_jv_testing: could not find a valid problem, skipping\n";
    s.cleanup();
    return;
  }

  // Save state at all time levels BEFORE the baseline run (for FD restore)
  decltype(e_real.m_state.m_w_i) w_i_init("w_i_init", nelemd);
  decltype(e_real.m_state.m_phinh_i) phi_init("phi_init", nelemd);
  decltype(e_real.m_state.m_v) v_init("v_init", nelemd);
  decltype(e_real.m_state.m_vtheta_dp) vth_init("vth_init", nelemd);
  decltype(e_real.m_state.m_dp3d) dp_init("dp_init", nelemd);
  Kokkos::deep_copy(w_i_init,  e_real.m_state.m_w_i);
  Kokkos::deep_copy(phi_init,  e_real.m_state.m_phinh_i);
  Kokkos::deep_copy(v_init,    e_real.m_state.m_v);
  Kokkos::deep_copy(vth_init,  e_real.m_state.m_vtheta_dp);
  Kokkos::deep_copy(dp_init,   e_real.m_state.m_dp3d);

  // --- Compute baseline output from DIRK ---
  dirk_real.run(nm1, alphadt_nm1, n0, alphadt_n0, np1, dt2,
                e_real, hvcoord, true /* BFB */);
  Kokkos::fence();
  const auto w_base   = cmvdc(e_real.m_state.m_w_i);
  const auto phi_base = cmvdc(e_real.m_state.m_phinh_i);

  // Restore state to the initial state for the FAD and FD runs
  Kokkos::deep_copy(e_real.m_state.m_w_i,       w_i_init);
  Kokkos::deep_copy(e_real.m_state.m_phinh_i,   phi_init);
  Kokkos::deep_copy(e_real.m_state.m_v,         v_init);
  Kokkos::deep_copy(e_real.m_state.m_vtheta_dp, vth_init);
  Kokkos::deep_copy(e_real.m_state.m_dp3d,      dp_init);

  // --- Set up FAD state by importing Real values at all time levels ---
  for (int tl = 0; tl < NUM_TIME_LEVELS; ++tl)
    elems_dx.m_state.import_values(e_real.m_state, tl);
  // m_derived.m_divdp_proj will be filled by run_initial_guess inside run_JV

  // --- Random perturbation vector V (stored in adj_state at n0) ---
  ElementsStateST<Real> adj_state;
  adj_state.init(nelemd);
  {
    const auto vm  = Kokkos::create_mirror_view(adj_state.m_v);
    const auto wm  = Kokkos::create_mirror_view(adj_state.m_w_i);
    const auto pm  = Kokkos::create_mirror_view(adj_state.m_phinh_i);
    const auto vtm = Kokkos::create_mirror_view(adj_state.m_vtheta_dp);
    const auto dpm = Kokkos::create_mirror_view(adj_state.m_dp3d);
    for (int ie = 0; ie < nelemd; ++ie)
      for (int i = 0; i < np; ++i)
        for (int j = 0; j < np; ++j) {
          for (int k = 0; k < nlev; ++k) {
            vm (ie,n0,0,i,j,k/dfi::packn)[k%dfi::packn] = r.urrng(-1,1);
            vm (ie,n0,1,i,j,k/dfi::packn)[k%dfi::packn] = r.urrng(-1,1);
            vtm(ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] = r.urrng(-1,1);
            dpm(ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] = r.urrng(-1,1);
            wm (ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] = r.urrng(-1,1);
            pm (ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] = r.urrng(-1,1);
          }
          // top interface level
          wm(ie,n0,i,j,nlev/dfi::packn)[nlev%dfi::packn] = r.urrng(-1,1);
          pm(ie,n0,i,j,nlev/dfi::packn)[nlev%dfi::packn] = r.urrng(-1,1);
        }
    Kokkos::deep_copy(adj_state.m_v,          vm);
    Kokkos::deep_copy(adj_state.m_w_i,        wm);
    Kokkos::deep_copy(adj_state.m_phinh_i,    pm);
    Kokkos::deep_copy(adj_state.m_vtheta_dp,  vtm);
    Kokkos::deep_copy(adj_state.m_dp3d,       dpm);
  }

  // Save the perturbation vector V (at n0) BEFORE calling run_JV, so the naming
  // stays unambiguous: V_* is the input perturbation, jv_* is the J*V output.
  const auto V_w   = cmvdc(adj_state.m_w_i);
  const auto V_phi = cmvdc(adj_state.m_phinh_i);
  const auto V_v   = cmvdc(adj_state.m_v);
  const auto V_vth = cmvdc(adj_state.m_vtheta_dp);
  const auto V_dp  = cmvdc(adj_state.m_dp3d);

  // --- Compute J*V using the FAD functor ---
  // run_JV writes only adj_state at np1; adj_state at n0 (V) is unchanged.
  dirk_dx.run_JV(nm1, alphadt_nm1, n0, alphadt_n0, np1, dt2,
                 elems_dx, hvcoord, adj_state);
  Kokkos::fence();

  // J*V is now in adj_state at np1
  const auto jv_w   = cmvdc(adj_state.m_w_i);
  const auto jv_phi = cmvdc(adj_state.m_phinh_i);

  // --- Compute J*V via finite differences for several step sizes ---
  const Real h_base = 1e-4;
  std::vector<Real> h_vals = {h_base, h_base*1e-1, h_base*1e-2, h_base*1e-3};
  std::vector<Real> err_vals;

  for (Real h : h_vals) {
    // Restore to initial state, then perturb n0: x_n0 + h*V
    {
      const auto wm  = Kokkos::create_mirror_view(e_real.m_state.m_w_i);
      const auto pm  = Kokkos::create_mirror_view(e_real.m_state.m_phinh_i);
      const auto vm  = Kokkos::create_mirror_view(e_real.m_state.m_v);
      const auto vtm = Kokkos::create_mirror_view(e_real.m_state.m_vtheta_dp);
      const auto dpm = Kokkos::create_mirror_view(e_real.m_state.m_dp3d);
      Kokkos::deep_copy(wm,  w_i_init);
      Kokkos::deep_copy(pm,  phi_init);
      Kokkos::deep_copy(vm,  v_init);
      Kokkos::deep_copy(vtm, vth_init);
      Kokkos::deep_copy(dpm, dp_init);
      for (int ie = 0; ie < nelemd; ++ie)
        for (int i = 0; i < np; ++i)
          for (int j = 0; j < np; ++j) {
            for (int k = 0; k < nlev; ++k) {
              wm (ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] +=
                h * V_w  (ie,n0,  i,j,k/dfi::packn)[k%dfi::packn];
              pm (ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] +=
                h * V_phi(ie,n0,  i,j,k/dfi::packn)[k%dfi::packn];
              vm (ie,n0,0,i,j,k/dfi::packn)[k%dfi::packn] +=
                h * V_v  (ie,n0,0,i,j,k/dfi::packn)[k%dfi::packn];
              vm (ie,n0,1,i,j,k/dfi::packn)[k%dfi::packn] +=
                h * V_v  (ie,n0,1,i,j,k/dfi::packn)[k%dfi::packn];
              vtm(ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] +=
                h * V_vth(ie,n0,  i,j,k/dfi::packn)[k%dfi::packn];
              dpm(ie,n0,  i,j,k/dfi::packn)[k%dfi::packn] +=
                h * V_dp (ie,n0,  i,j,k/dfi::packn)[k%dfi::packn];
            }
            // top interface level nlev
            wm(ie,n0,i,j,nlev/dfi::packn)[nlev%dfi::packn] +=
              h * V_w  (ie,n0,i,j,nlev/dfi::packn)[nlev%dfi::packn];
            pm(ie,n0,i,j,nlev/dfi::packn)[nlev%dfi::packn] +=
              h * V_phi(ie,n0,i,j,nlev/dfi::packn)[nlev%dfi::packn];
          }
      Kokkos::deep_copy(e_real.m_state.m_w_i,       wm);
      Kokkos::deep_copy(e_real.m_state.m_phinh_i,   pm);
      Kokkos::deep_copy(e_real.m_state.m_v,         vm);
      Kokkos::deep_copy(e_real.m_state.m_vtheta_dp, vtm);
      Kokkos::deep_copy(e_real.m_state.m_dp3d,      dpm);
    }
    ...
    const auto w_pert   = cmvdc(e_real.m_state.m_w_i);
    const auto phi_pert = cmvdc(e_real.m_state.m_phinh_i);

    // Compute max relative error between FD and FAD J*V
    Real max_err = 0;
    for (int ie = 0; ie < nelemd; ++ie)
      for (int i = 0; i < np; ++i)
        for (int j = 0; j < np; ++j)
          for (int k = 0; k <= nlev; ++k) {
            const Real fd_w   = (ADValue(w_pert  (ie,np1,i,j,k/dfi::packn)[k%dfi::packn])
                               - ADValue(w_base  (ie,np1,i,j,k/dfi::packn)[k%dfi::packn])) / h;
            const Real fd_phi = (ADValue(phi_pert(ie,np1,i,j,k/dfi::packn)[k%dfi::packn])
                               - ADValue(phi_base(ie,np1,i,j,k/dfi::packn)[k%dfi::packn])) / h;
            const Real jv_w_val   = jv_w  (ie,np1,i,j,k/dfi::packn)[k%dfi::packn];
            const Real jv_phi_val = jv_phi(ie,np1,i,j,k/dfi::packn)[k%dfi::packn];
            const Real ref_w   = std::abs(jv_w_val)   + 1e-10;
            const Real ref_phi = std::abs(jv_phi_val) + 1e-10;
            max_err = std::max(max_err, std::abs(fd_w   - jv_w_val  ) / ref_w  );
            max_err = std::max(max_err, std::abs(fd_phi - jv_phi_val) / ref_phi);
          }
    err_vals.push_back(max_err);
  }

  // Check that the minimum FD error is sufficiently small
  const Real min_err = *std::min_element(err_vals.begin(), err_vals.end());
  printf("dirk_jv_testing: FD errors = [");
  for (size_t i = 0; i < err_vals.size(); ++i)
    printf(" %9.2e", err_vals[i]);
  printf(" ], min = %9.2e\n", min_err);

  REQUIRE(min_err < 1e-3);
